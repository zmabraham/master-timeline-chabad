...
Modify `buildTimeline` to accept a `groupBy` strategy. Replace the `groups` const with a `setGroups` API on the returned handle.

```typescript
// add to types in timeline.ts:
...
(Full integration involves passing `groupBy` mode into `buildTimeline` + `refresh()` so items pick the right group. Keep the implementation minimal — see existing pattern in timeline.ts.)
